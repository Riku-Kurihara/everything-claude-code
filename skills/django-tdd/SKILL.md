...
pytest、factory_boy、Django REST Frameworkを使用したDjangoアプリケーション向けテスト駆動開発。
...
- Djangoプロジェクト向けテストインフラストラクチャをセットアップするとき
...
# ステップ2: GREEN - テストを成功させる
# ユーザーモデルまたはファクトリを作成

# ステップ3: REFACTOR - テストを成功させながら改善する
```
...
class TestPaymentView:
    """モック支払いゲートウェイでの支払いビューをテスト。"""

    @patch('apps.payments.services.stripe')
    def test_successful_payment(self, mock_stripe, client, user, product):
        """モック Stripe での成功支払いをテスト。"""
        # モックを構成
...
- **ファクトリを使用**: 手動でのオブジェクト作成の代わり
- **テストごとに1つのアサーション**: テストを焦点を当てる
- **説明的なテスト名**: `test_user_cannot_delete_others_post`
- **エッジケースをテスト**: 空の入力、None値、境界条件
- **外部サービスをモック**: 外部APIに依存しない
- **フィクスチャを使用**: 重複を排除
- **パーミッションをテスト**: 認可が機能していることを確認
- **テストを高速に保つ**: `--reuse-db` と `--nomigrations` を使用

### DON'T

- **Djangoの内部をテストしない**: Djangoが機能することを信頼
- **サードパーティコードをテストしない**: ライブラリが機能することを信頼
- **失敗テストを無視しない**: すべてのテストが成功する必要がある
- **テストに依存性を作らない**: テストは任意の順序で実行できるべき
- **過度にモックしない**: 外部依存関係のみモック
- **非公開メソッドをテストしない**: 公開インターフェースをテスト
...
覚えておくこと: テストはドキュメントです。良いテストはコードがどのように動作するべきかを説明します。シンプルで、読みやすく、保守可能に保ってください。
