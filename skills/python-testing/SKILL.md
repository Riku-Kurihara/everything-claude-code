...
pytestとベストプラクティスを使用したPythonアプリケーションの包括的なテスト戦略。
...
1. **RED**: 希望する動作に対する失敗するテストを書く
2. **GREEN**: テストを成功させるための最小限のコードを書く
...
assert result is False  # 厳密にFalse
...
- **80%以上のカバレッジを目指す**: 重要なパスに焦点
- **テストを高速に保つ**: マークを使用して遅いテストを分離

### してはいけないこと

- **実装をテストしない**: 内部ではなく動作をテスト
- **テストで複雑な条件を使用しない**: テストをシンプルに保つ
- **テスト失敗を無視しない**: すべてのテストは成功すべき
- **サードパーティコードをテストしない**: ライブラリが機能すると信頼
- **テスト間で状態を共有しない**: テストは独立すべき
- **テストで例外をキャッチしない**: `pytest.raises`を使用
- **printステートメントを使用しない**: アサーションとpytestの出力を使用
- **非常に脆いテストを書かない**: 過度に具体的なモックを回避

## 一般的なパターン

### APIエンドポイントテスト (FastAPI/Flask)
...
| `@patch()` | 関数とクラスをモック |
| `tmp_path` フィクスチャ | 自動テンポラリディレクトリ |
| `pytest --cov` | カバレッジレポート生成 |
| `assert` | シンプルで読みやすいアサーション |

**覚えておいてください**：テストはコードです。クリーン、読みやすく、保守しやすく保ってください。良いテストはバグをキャッチします。素晴らしいテストはバグを防ぎます。
